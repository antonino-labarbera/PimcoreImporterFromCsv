...
#[AsCommand(
    name: 'app:company-importer',
    description: 'Imports companies from a CSV file.',
)]
class CompanyImporterCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $this->companyImporter->importCompanies($csvFilePath);
            $output->writeln('Companies imported successfully.');
            return Command::SUCCESS;
        } catch (\Exception $e) {
            $output->writeln('<error>Failed to import companies: ' . $e->getMessage() . '</error>');
            return Command::FAILURE;
        }
        return Command::SUCCESS;
    }
